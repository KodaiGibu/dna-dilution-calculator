...
| `load_presets()` / `save_presets()` | `localStorage["dna_dilution_presets"]`（初期値は空リスト） |
...
`GenePresetDialog`・`ColumnSelectDialog`・`DNADilutionApp` の各メソッドは、対応関係がわかるよう
`src/app.js` 内にコメント（例: `// _refresh_tree_nm`）で明示しています。

## 機能

- **モル濃度希釈 (nM)**: ターゲット nM・サンプル量・MW per bp・共通/個別塩基長、塩基長プリセット
- **濃度希釈 (ng/µL)**: ターゲット ng/µL・サンプル量
- **入力**: 個別入力（列ごとに貼り付け）／一括入力（タブ・カンマ区切り、ヘッダ行は自動スキップ）
- **塩基長プリセット管理**: 追加・上書き保存・削除（同名かつ同 bp の重複はブロック）
...
```
.
├── index.html              # UI（モード2タブ + 入力2タブ + 結果表 + ステータスバー）
├── assets/
│   ├── styles.css
│   └── favicon.svg
├── src/
│   ├── calc.js             # 計算コア（Python 互換の丸め・文字列化・パース）
│   └── app.js              # UI・プリセット・列選択・CSV/HTML/クリップボード出力
├── tests/
│   └── calc.test.mjs       # Python 版との一致検証
...
git init
git add .
git commit -m "feat: DNA希釈計算ツール v2.2 Web版（tkinter版からの移植）"
git branch -M main
...
`npm test` で以下を検証しています（Python 実行結果と突き合わせ済み）。
